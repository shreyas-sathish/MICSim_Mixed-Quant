...
class Layer():
    ...
    #     self.input_param = config_object.input_param
    #     # self.tech = tech
    #     self.tech = config_object.tech
    #     # self.cell = cell
    #     if self.conf.modeltype == 'transformer_kq': 
    #          self.cell = config_object.DMVMKQCell
    #     elif self.conf.modeltype == 'transformer_pv': 
    #          self.cell = config_object.DMVMPVCell
    #     else:
    #          # Default to StaticMVMCell for CNNs
    #          self.cell = config_object.StaticMVMCell
    #     self.Tile = None
    #     self.AdderTree = neurosim.AdderTree(self.input_param,self.tech,self.cell)
    #     self.ReLu =  neurosim.BitShifter(self.input_param,self.tech,self.cell)
    #     self.AdderTreeshift = neurosim.AdderTree(self.input_param,self.tech,self.cell)
    #     self.debug_mode = 1
# In Performance/src/CNN/Layer_mixed.py

    def __init__(self,config_object):
        # --- START OF "RUN-AT-ALL-COSTS" FIX ---

        # 1. Create a SINGLE, DEFAULT 8-bit config
        # We will use this for EVERYTHING to stop the conflict.
        dupl_conf = configuration()
        dupl_conf.settings_generation()

        # 2. Store this 8-bit config as self.conf
        # Layer.Map() will now use this 8-bit config.
        self.conf = dupl_conf

        # 3. ***THIS IS THE FIX***
        #    Define the variables from dupl_conf before using them
        dupl_input_param = dupl_conf.input_param
        dupl_tech = dupl_conf.tech
        
        # 4. Select the correct 8-bit cell
        # We use config_object ONE TIME here, just to get the model type
        if config_object.modeltype == 'transformer_kq': 
             dupl_cell = dupl_conf.DMVMKQCell
        elif config_object.modeltype == 'transformer_pv': 
             dupl_cell = dupl_conf.DMVMPVCell
        else:
             dupl_cell = dupl_conf.StaticMVMCell

        # 5. Store the 8-bit C++ objects from it
        self.input_param = dupl_input_param
        self.tech = dupl_tech
        self.cell = dupl_cell

        # 6. Initialize all NeuroSim components with these 8-bit objects
        self.Tile = None
        self.AdderTree = neurosim.AdderTree(self.input_param, self.tech, self.cell)
        self.ReLu =  neurosim.BitShifter(self.input_param, self.tech, self.cell)
        self.AdderTreeshift = neurosim.AdderTree(self.input_param, self.tech, self.cell)
