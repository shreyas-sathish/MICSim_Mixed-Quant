import neurosim
import numpy as np
from Performance.src.CNN.Layer_mixed import Layer
...
class Network():
    ...
    def Map(self):
        ...
        for layer_i, layer_structure in enumerate(self.layer_list):
            ...
            # --- START: NEW MIXED-PRECISION LOGIC ---
            # 1. Create a new, layer-specific configuration object
            layer_conf = configuration()
            
            # 2. Assign the layer-specific precision from the layer_list
            #    (Indices 13 and 14, after the layer type 'Conv'/'FC')
            try:
                layer_conf.numBitWeight = layer_structure[-2]
                layer_conf.numBitInput = layer_structure[-1]
            except IndexError:
                print(f"ERROR: Layer '{layer_structure[11]}' does not have precision bits specified.")
                print("Please add wbits and abits to all layers in main_resnet18.py")
                raise
                
            # 3. Generate hardware parameters (input_param, tech, cell)
            #    based on THIS layer's precision
            layer_conf.settings_generation()
            # --- END: NEW MIXED-PRECISION LOGIC ---

            if layer_structure[-3] == 'Conv' or layer_structure[-3] == 'FC': # Check index -3 for type
                print(layer_structure)
                
                # 4. Create the Layer object using the new LAYER-SPECIFIC parameters
                # layer = Layer(layer_conf.input_param, 
                #               layer_conf.tech, 
                #               layer_conf.StaticMVMCell)
            ...
            elif layer_structure[-3] == 'MatmulKQ' or layer_structure[-3] == 'MatmulPV': # Check index -3 for type
                
                if layer_structure[-3] == 'MatmulKQ':
                    # Use the LAYER-SPECIFIC DMVMKQCell
                    layer = Layer(layer_conf.input_param,
                                  layer_conf.tech,
                                  layer_conf.DMVMKQCell)
                else:
                    # Use the LAYER-SPECIFIC DMVMPVCell
                    layer = Layer(layer_conf.input_param,
                                  layer_conf.tech,
                                  layer_conf.DMVMPVCell)                       
        ...
        for l in self.layer_list:
            if  l[-3] not in self.layer_type: # Check index -3 for type
                self.layer_type.append(l[-3])
