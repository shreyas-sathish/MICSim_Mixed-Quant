...
def Conv_(input, inputshift, weight, weightshift, stride, padding, name = None,dumpname=None):
    output = None
    ...
    # use weight stationary dataflow
    for i in range(filter_size[2]):     # size[2]: height of kernel;   k1 
        for j in range(filter_size[3]): # size[3]: width of kernel;    k2
            partial_output_dec = None
                                        # size[1]: # input channel;    D1
            for k in range(int(np.ceil(filter_size[1] / ram_depth))):
                start_channel = k * ram_depth
                end_channel = min((k + 1) * ram_depth, filter_size[1])
                # mvin the weight buffer and input buffer
                weight_section = weight[:, start_channel:end_channel, i, j].clone()
                input_section  =  input[:, start_channel:end_channel, :, :].clone()
                input_dummy = torch.ones_like(input[0:1, start_channel:end_channel, 0:1, 0:1])
                weight_dummy_ones = torch.ones_like(weight[0:1, start_channel:end_channel, 0, 0])
                ...
                for input_b, scale_in, inputshift_b in zip(input_section_bins, input_section_scales, inputshift_section_bins):
                    for l, (weight_b, scale_w, weightshift_b) in enumerate(zip(weight_section_bins, weight_section_scales,weightshift_section_bins)):
                        ...
                        # VMM in CIM
                        # here we use digit input due to the adc will convert digit input into voltage "0" and "1"
                        # todo flag: we need to implement the adc here
                        partial_output_b_sum = torch.mm(weight_b,input_b.transpose(0, 1).reshape(
                                                                (end_channel - start_channel),-1)).reshape(
                                filter_size[0], input_size[0], input_size[2], input_size[3]).transpose(0, 1)


                        if not skip_ref1 :
                            #if inputshift_b there is no need to apply it to the array
                            dummy_partial_output_b_sum1 = torch.mm(weight_b,(input_dummy * inputshift_b).transpose(0, 1).reshape(
                                                                    (end_channel - start_channel),-1)).reshape(
                                    filter_size[0], 1, 1, 1).transpose(0, 1)

                        if not skip_ref2:
                            # the ref2 could be used to shift the weight but not necessarily cancel the gmin of the normal array with the same input.
                            # the gmin cancel is used when view ref2 as unsign and the shift and main array relation is minus (minus outside). in general case, shift is not necessary negativ. thus split the function of ref 2 and gmin cancel
                            ...
                            if DigitRef2 == "True":
                               dummy_partial_output_b_sum2 = weightshift_b * input_b.sum(axis=1,keepdims=True)
                                ...
                                dummy_partial_output_b_sum2 = torch.mm(weight_dummy, input_b.transpose(0, 1).reshape(
                                    (end_channel - start_channel), -1)).reshape(
                                            1 , input_size[0], input_size[2], input_size[3]).transpose(0, 1)
                        ...
                        if not skip_ref3:
                            if DigitRef3 == "True":
                                ...
                                dummy_partial_output_b_sum3 = torch.mm(weight_dummy, (input_dummy * inputshift_b).transpose(0, 1).reshape(
                                    (end_channel - start_channel), -1)).reshape(
                        ...
                        if not skip_gmin_dummy1 or not skip_gmin_dummy2:
                            ...
                            if not skip_gmin_dummy1:
                                dummy_partial_output_b_gmin = torch.mm(weight_dummy, input_b.transpose(0, 1).reshape(
                                    (end_channel - start_channel), -1)).reshape(
                                            1 , input_size[0], input_size[2], input_size[3]).transpose(0, 1)
                            if not skip_gmin_dummy2:
                                dummy_partial_output_b_gmin2 = torch.mm(weight_dummy, (input_dummy * inputshift_b).transpose(0,  1).reshape(
                                (end_channel - start_channel), -1)).reshape(
                                         1, 1, 1, 1).transpose(0, 1)
                        ...
                        if DumpData == "True":
                            ...
                            if not skip_ref1:
                                ...
                                if d1_array is None:
                                    ...
                                    # d1_array = np.append(d1_array, d1)    
                        ...
                        # shiftadd
                        if partial_output_dec is None:
                            partial_output_dec = partial_output_b_sum * scale_w * scale_in
                            #partial_output_dec_clean = partial_output_b_sum_clean * scale_w * scale_in
                        else:
                            partial_output_dec = partial_output_dec + partial_output_b_sum * scale_w * scale_in

            partial_output_dec = m(partial_output_dec)
            if output is None:
                output = partial_output_dec[:, :, i:i + o_height, j:j + o_width]
            else:
                output = output + partial_output_dec[:, :, i:i + o_height, j:j + o_width]
