...
# default fallbacks (in case CONFIG is not set)
QuantizationMode    = "WAGEV2"
hardware            = "False"
...
if config is not None and 'Quantization' in config:
    QuantizationMode    = config['Quantization'].get('mode', QuantizationMode)
    hardware            = config['Quantization'].get('hardware', hardware)
    weightsignmapping   = config['Quantization'].get('weightsignmapping', weightsignmapping)
    inputsignmapping    = config['Quantization'].get('inputsignmapping', inputsignmapping)
    weightmapping       = config['Quantization'].get('weightmapping', weightmapping)
    inputmapping        = config['Quantization'].get('inputmapping', inputmapping)
    dumpaveragevalue    = config['Quantization'].get('dumpaveragevalue', dumpaveragevalue)
if config is not None and 'ADC' in config:
    adc_mode            = config['ADC'].get('mode', adc_mode)
if config is not None and 'CIM' in config:
    cellprecision       = int(config['CIM'].get('cellprecision', cellprecision))
...
# from Accuracy.config.resnet18.config_resnet18_mixed import layer_quant_config
...
def get_layer_quant_params(layer_name):
    for name, w_bits, a_bits, scale in layer_quant_config:
        if name == layer_name:
            return w_bits, a_bits, scale
    return None, None, 1.0  # fallback
...
class QLinear(nn.Linear):
    ...
    def forward(self, input):    
        ...
        # Apply per-layer scaling if defined
        # scale_factor = getattr(self, 'scaling', 1.0)
        # weight_for_compute = self.weight * scale_factor
        # output = CIM_Linear.apply(
        #     input, weight_for_compute, self.bias, self.scale,
        #     self.quantize_weight, self.quantize_input, self.quantize_error,
        #     self.name, self.quantizer, self.training
        # )        
        # return output
        # --- Per-layer scaling and quantization override ---
        scale_factor = getattr(self, 'scaling', 1.0)
        weight_bits  = getattr(self, 'weight_bits', None)
        act_bits     = getattr(self, 'act_bits', None)

        # Temporarily adjust quantizer bit settings
        if weight_bits is not None:
            old_wbits = getattr(self.quantizer, 'w_bits', None)
            self.quantizer.w_bits = weight_bits
        if act_bits is not None:
            old_abits = getattr(self.quantizer, 'a_bits', None)
            self.quantizer.a_bits = act_bits
        ...
        # Restore quantizer bit settings
        if weight_bits is not None: self.quantizer.w_bits = old_wbits
        if act_bits is not None: self.quantizer.a_bits = old_abits
