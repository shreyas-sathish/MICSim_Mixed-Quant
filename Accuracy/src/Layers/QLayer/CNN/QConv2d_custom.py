...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        # output = CIM_Conv.apply(
        #     input, weight_for_compute, self.bias, self.stride[0], self.padding[0],
        #     self.dilation, self.groups, self.scale, self.quantize_weight,
        #     self.quantize_input, self.quantize_error, self.name, self.quantizer, self.training
        # )        
        # return output
        # --- Per-layer scaling and quantization override ---
        scale_factor = getattr(self, 'scaling', 1.0)
        weight_bits  = getattr(self, 'wbits', None)
        act_bits     = getattr(self, 'abits', None)

        # Temporarily adjust quantizer settings if layer-specific bits are defined
        ...
        # Restore quantizer bit settings (to avoid polluting next layer)
