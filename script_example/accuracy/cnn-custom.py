...
if n == 2:
    ...
    os.environ['CONFIG'] = './Accuracy/config/resnet18/config_resnet18_mixed_prec.ini'
# ====================init config path===========================
...
# layerwise_bits = []
# if 'LayerwiseQuantization' in config:
#     try:
#         layerwise_bits = ast.literal_eval(config['LayerwiseQuantization']['layers'])
#         logger(f"Loaded layer-wise quantization: {layerwise_bits}")
#     except Exception as e:
#         logger(f"Error reading LayerwiseQuantization section: {e}")
logger('===============================configurations===============================')
...
# Layerwise quantization support
# Load per-layer bitwidths and scaling from config (expected as Python list string)
layerwise_bits = []
if 'Quantization' in config and 'layer_quant_config' in config['Quantization']:
    layerwise_bits = ast.literal_eval(config['Quantization']['layer_quant_config'])

infer = Tester(model.cuda(), train_loader, test_loader, layerwise_bits=layerwise_bits)
